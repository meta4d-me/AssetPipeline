...
public:
	...
	static TQuaternion<T> FromRollPitchYaw(T roll, T pitch, T yaw)
	{
		...
		T sinRoll = std::sinf(Math::DegreeToRadian<T>(rollNoWinding) * 0.5f);
		T cosRoll = std::cosf(Math::DegreeToRadian<T>(rollNoWinding) * 0.5f);
		T sinPitch = std::sinf(Math::DegreeToRadian<T>(pitchNoWinding) * 0.5f);
		T cosPitch = std::cosf(Math::DegreeToRadian<T>(pitchNoWinding) * 0.5f);
		T sinYaw = std::sinf(Math::DegreeToRadian<T>(yawNoWinding) * 0.5f);
		T cosYaw = std::cosf(Math::DegreeToRadian<T>(yawNoWinding) * 0.5f);
